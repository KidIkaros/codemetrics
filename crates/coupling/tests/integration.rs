...
fn test_min_coupling_5_filter() {
    ...
    assert!(
        stdout.contains("MODULE COUPLING ANALYSIS"),
        "Missing header"
    );
    ...
    assert!(
        stdout.contains("INSTABILITY"),
        "Missing INSTABILITY column header"
    );
    assert!(stdout.contains("STATUS"), "Missing STATUS column header");
    assert!(
        stdout.contains("Total modules"),
        "Missing 'Total modules' in summary"
    );
    assert!(
        stdout.contains("Total dependencies"),
        "Missing 'Total dependencies' in summary"
    );
}
...
fn test_module_listing_format() {
    ...
    let output = coupling_cmd().arg(FIXTURE_PROJECT).output().unwrap();
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_most_coupled_section() {
    ...
    if stdout.contains("MOST COUPLED") {
        ...
        assert!(
            stdout.contains("fan-in:"),
            "Missing 'fan-in:' in MOST COUPLED section"
        );
        assert!(
            stdout.contains("fan-out:"),
            "Missing 'fan-out:' in MOST COUPLED section"
        );
    }
}
...
fn test_status_indicators() {
    ...
    let has_status =
        stdout.contains("low") || stdout.contains("moderate") || stdout.contains("high");
