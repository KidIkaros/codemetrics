...
fn scan_file(path: &str, check_unreachable: bool) -> Vec<DeadCodeFinding> {
    ...
    let lang_ext = Path::new(path).extension().and_then(|e| e.to_str()).unwrap_or("");
    ...
    for (i, &line) in lines.iter().enumerate() {
        ...
        if trimmed.contains("#[allow(dead_code)]") || trimmed.contains("# noqa: F401") {
            findings.push(DeadCodeFinding {
                ...
                suggested_fix: Some("Remove the dead code instead of suppressing the warning.".to_string()),
            });
        }
        ...
        match lang_ext {
            "py" => {
                ...
                // We use a simple heuristic: if the imported name never appears again in the file
        ...
        if check_unreachable {
            ...
            if trimmed == "if true {" || trimmed == "if (true) {" {
                findings.push(DeadCodeFinding {
                    ...
                    context: truncate(trimmed, 80).to_string(),
                    severity: "medium".to_string(),
                    suggested_fix: Some("Remove the `if true` wrapper — the body always executes.".to_string()),
                });
            }
        }

        // 4. Dead assignments: `let _ = expr;` — discarding a value silently
        if (lang_ext == "rs" && trimmed.starts_with("let _ ="))
            || (lang_ext == "rs" && trimmed.starts_with("let _x ="))
        {
            findings.push(DeadCodeFinding {
                file: path.to_string(),
                line: lineno,
                ...
                context: truncate(trimmed, 80).to_string(),
                severity: "low".to_string(),
                suggested_fix: Some("Consider handling the value instead of discarding it with `let _ = ...`.".to_string()),
            });
        }
        ...
        if (trimmed == "{}" || trimmed == "{ }") && i > 0 {
            ...
            if is_fn_or_branch {
                findings.push(DeadCodeFinding {
                    ...
                    severity: "low".to_string(),
                    suggested_fix: Some("Add implementation or a `todo!()` / `pass` to document intent.".to_string()),
                });
            }
        }
    }
...
fn extract_python_import_name(line: &str) -> Option<String> {
    ...
    if let Some(rest) = line.strip_prefix("from ") {
        if let Some(pos) = rest.find(" import ") {
            ...
            let name = names.split(',').next()?.trim().split_whitespace().next()?.trim_matches(|c: char| !c.is_alphanumeric() && c != '_');
            if !name.is_empty() { return Some(name.to_string()); }
        }
    } else if let Some(rest) = line.strip_prefix("import ") {
        let name = rest.split(',').next()?.trim().split_whitespace().next()?.trim_matches(|c: char| !c.is_alphanumeric() && c != '_');
        if !name.is_empty() { return Some(name.to_string()); }
    }
    None
}
...
fn extract_js_import_names(line: &str) -> Vec<String> {
    ...
    if let Some(start) = line.find('{') {
        ...
        let name = after_import.split_whitespace().next().unwrap_or("").trim_matches('*');
...
fn run(cli: Cli) {
    ...
    let allow_dc = all_findings.iter().filter(|f| f.kind == "allow(dead_code)").count();
    let unused_imp = all_findings.iter().filter(|f| f.kind == "unused_import").count();
    let unreachable = all_findings.iter().filter(|f| f.kind == "unreachable_branch").count();
    let dead_assign = all_findings.iter().filter(|f| f.kind == "dead_assignment").count();
    let empty_blk = all_findings.iter().filter(|f| f.kind == "empty_block").count();
    ...
    match cli.format.as_str() {
        "json" => {
            let report = DeadCodeReport { findings: all_findings, summary };
        ...
        _ => {
            if all_findings.is_empty() {
                ...
                let cols = vec![
                    ...
                    Column { header: "Kind", width: 20, align_right: false },
...
mod tests {
    ...
    fn test_extract_python_import() {
        ...
        assert_eq!(extract_python_import_name("from os import path"), Some("path".into()));
        assert_eq!(extract_python_import_name("from os.path import join"), Some("join".into()));
    }
